...
function renderChart(metric = 'temperature') {
  ...
  const xStep = values.length > 1 ? 630 / (values.length - 1) : 630;
  const points = values.map((value, index) => `${35 + index * xStep},${185 - ((value - min) / range) * 145}`).join(' ');
  const area = `35,185 ${points} 665,185`;
  const labels = sensorHistory.map((item, index) => index % Math.ceil(sensorHistory.length / 6) === 0 ? `<text class="chart-label" x="${25 + index * xStep}" y="210">${item.recorded_at?.slice(11, 16) || item.time}</text>` : '').join('');
  svg.innerHTML = `<line class="grid-line" x1="35" y1="40" x2="665" y2="40"/><line class="grid-line" x1="35" y1="112" x2="665" y2="112"/><line class="grid-line" x1="35" y1="185" x2="665" y2="185"/><polygon class="chart-area" points="${area}"/><polyline class="chart-line" points="${points}"/>${values.map((value, index) => `<circle class="chart-dot" cx="${35 + index * xStep}" cy="${185 - ((value - min) / range) * 145}" r="3"/>`).join('')}${labels}`;
}
...
function applyOverview(data, history, vehicleData, sensorLatest) {
  ...
  setText('temperature', `${sensorLatest.temperature} °C`); setText('humidity', `${sensorLatest.humidity} %`); setText('pressure', `${sensorLatest.pressure} hPa`); setText('gasResistance', `${(sensorLatest.gas_resistance / 1000).toFixed(1)} kΩ`);
...
async function loadDashboard(storeId) {
  ...
  const [overview, history, vehicleData, sensorLatest] = await Promise.all([api(`/api/overview${suffix}`), api('/api/predictions/history'), api('/api/vehicles'), api('/api/sensors/latest')]);
  applyOverview(overview, history, vehicleData, sensorLatest);
}
